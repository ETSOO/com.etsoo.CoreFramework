...
namespace Tests.MessageQueue
{
    ...
    public class LocalRabbitMQTests
    {
        ...
        public async Task ProducerReceiveWithDLXAsyncTest()
        {
            ...
            var action = (MessageReceivedProperties properties, SimpleData? data) =>
            {
                var header = properties.Headers;

                if (header?.TryGetValue(LocalRabbitMQConsumer.RetryCountField, out var retryCount) is not true)
                {
                    throw new Exception("No retry count header");
                }
